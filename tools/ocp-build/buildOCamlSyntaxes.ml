...
let get_pp lib basename envs =
  let pp_flags =
    List.map (fun s -> S s)
      (ppflags_option.get envs) in
  match  syntax_option.get envs with
  |  [] ->
    let pp_requires =  pp_requires_option.get envs in
    let pp_option = pp_option.get envs in
